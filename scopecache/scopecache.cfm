<!---
	...
	Last Updated : December 5, 2011 (v5)
	...
	deferreddata:	This is a string that points to a function that can be called from within scope cache. So for example, request.someudf(5). The
					evaluate function is used. This feature is necessary when you want to use the result of a method as the data and not have that
					method called every time scopecache itself is run.
	...
	// new in v5
	path:			specifies a path to a file, which will be read in to the cache directly
	isBinary:		when true, reads the file in as binary.
...
<cfif not dontRun>
	<cfif thisTag.executionMode is "start">
		...
		<cfif attributes.scope neq "file" and structKeyExists(ptr.scopeCache,attributes.name) and NOT StructIsEmpty(ptr.scopeCache[attributes.name])>
	...
	<cfelse>
		...
		<cfif attributes.scope neq "file">								
			<cflock type="exclusive" scope="#attributes.scope#" timeout="30">
				<cfset ptr.scopeCache[attributes.name] = structNew()>
	
				<cfif isDefined("attributes.deferreddata")>
					<cfset attributes.data = evaluate(attributes.deferreddata)>
				</cfif>
	
				<cfif not isDefined("attributes.data")>
					<cfif isDefined("attributes.path")>
						<cffile action="#iif(isDefined('attributes.isBinary') AND attributes.isBinary,de('readBinary'),de('read'))#" 
								file="#attributes.path#" 
								variable="fileGeneratedContent" />
						<cfset ptr.scopeCache[attributes.name].value = fileGeneratedContent />
					<cfelse>
		...
		<cfelse>
			<cflock name="#attributes.file#" type="exclusive" timeout="30">
				...
				<cfif not isDefined("attributes.data")>
					<cfif isDefined("attributes.path")>
						<cffile action="#iif(Evaluate(isDefined('attributes.isBinary') AND attributes.isBinary),de('readBinary'),de('read'))#" 
								file="#attributes.path#" 
								variable="fileGeneratedContent" />
						<cfset data.value = fileGeneratedContent />
					<cfelse>
